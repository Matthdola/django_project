...
class Author(models.Model):
	...
	def get_absolute_url(self):
		return reverse('djangobin:trending_snippets',args=[self.user.username])

	def get_snippet_count(self):
		return self.user.snippet_set.count()

	#class Meta:
		#unique_together = (('self.user.username','email'),)
...
class Snippet(models.Model):
	...
	highlighted_code = models.TextField(blank=True,help_text="Read only field. Will"
		+" code syntax-highlited version of the original code.")
	...
	hits = models.IntegerField(default=0, help_text="Read only field."
		+"Will update the hits")
	slug = models.SlugField(help_text="Read only filed. Will be filled automatically")
	created_on = models.DateTimeField(auto_now_add=True)
